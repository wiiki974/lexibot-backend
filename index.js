...
// Fonction pour extraire dynamiquement les sections de réponse
function extraireSections(texte) {
  ...
  for (let i = 0; i < labels.length; i++) {
    const titre = parties[i * 2 + 1];
    const contenu = parties[i * 2 + 2];
    if (titre && contenu) {
      sections[labels[i]] = `${i + 1}. ${titre.trim()}\n${contenu.trim()}`;
    }
  }

  // Recherche des modèles de lettres
...
app.post("/generer", async (req, res) => {
  ...
  if (!message || typeof message !== "string") {
    return res.status(400).json({ erreur: "Message manquant ou invalide" });
  }
